...
export default function SignupPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      setTimeout(() => {
        setIsOtpLoading(false);
        handleSuccessfulAuth(name || `Teacher (+91 ${phone.slice(-4)})`);
      }, 800);
      return;
    }
        ...
        <div className="hidden lg:flex flex-1 relative bg-[#E1A140] flex-col justify-between p-12 overflow-hidden min-h-[680px]">
          ...
          <div className="relative z-10 max-w-lg mx-auto text-center mt-auto bg-black/30 backdrop-blur-md p-6 rounded-3xl border border-white/20">
            ...
            <p className="text-amber-200 text-xs font-bold mt-2 uppercase tracking-wider">
              100% Free Forever for Individual Educators
            </p>
          </div>
        </div>
          ...
          <div className="w-full max-w-[440px] bg-white sm:border sm:border-slate-200 p-6 sm:p-8 rounded-3xl sm:shadow-xl space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="text-center">
                ...
                >
                  {authMethod === "email" ? "← Switch to Phone / QR Registration" : "Or Create Account with Email & Password"}
                </button>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {!otpSent ? (
                  <form onSubmit={handleSendOtp} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        required
                        className="w-full min-h-[44px] px-4 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-transparent text-slate-900 font-bold text-xs mb-3"
                      />
                  ...
                  <form onSubmit={handleVerifyOtp} className="space-y-4">
                    ...
                    >
                      {isOtpLoading ? "Verifying Account..." : "Create Account & Access Free Kits 🚀"}
                    </Button>
            ...
            <p className="text-center text-xs text-slate-600 font-bold pt-2 border-t border-slate-100">
              {t('has_account')}{" "}
              <Link href="/login" className="text-emerald-800 font-black hover:underline">
                {t('log_in')}
              </Link>
            </p>
          </div>
