              ...
              >
                ...
                <div className="flex justify-between items-end border-b border-slate-200 pb-2">
                  <div className="space-y-1">
                    <Link href={`/content/${activeClass.toLowerCase().replace(" ", "-")}/${subject.name.toLowerCase().replace(/\s+/g, "-")}`} className="flex items-center gap-2 group">
                      <span className={`w-3 h-3 rounded-full bg-gradient-to-r ${subject.color}`} />
                      <h2 className="text-2xl font-bold font-serif tracking-tight text-slate-800 group-hover:text-emerald-800 transition-colors flex items-center gap-2">
                        {subject.name}
                        <ChevronRight className="w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity text-emerald-700" />
                      </h2>
                    </Link>
                  </div>
                  <div className="flex items-center gap-3">
                    <Link
                      href={`/content/${activeClass.toLowerCase().replace(" ", "-")}/${subject.name.toLowerCase().replace(/\s+/g, "-")}`}
                      className="text-xs font-bold text-emerald-700 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-lg transition-colors border border-emerald-200/60"
                    >
                      Subject Hub &rarr;
                    </Link>
                      ...
                      return (
                        <div key={book.id} className="relative z-10">
                          ...
                          {book.isLocked ? (
                            ...
                            <Link href={`/content/${activeClass.toLowerCase().replace(" ", "-")}/${subject.name.toLowerCase().replace(/\s+/g, "-")}/chapter-${book.id}`}>
                              <motion.div
                                onMouseEnter={(e) => handleMouseEnter(e, book, bookKey)}
                                onMouseLeave={() => setHoveredBook(null)}
                                whileHover={{ 
