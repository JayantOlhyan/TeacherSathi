...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!auth.context.canViewScope(scopeType, scopeId)) {
      return NextResponse.json({ error: 'Forbidden: Insufficient permissions for this scope' }, { status: 403 });
    }

    const supabase = createClient();
    const [academicData, gapsData] = await Promise.all([
      reportingService.getAcademicMetrics(scopeType, scopeId, supabase),
      reportingService.getLearningGapSummary(scopeType, scopeId, supabase),
    ]);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        scopeId,
        ...academicData,
        learningGaps: gapsData,
      },
    });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Failed to fetch academic intelligence';
