...
const UpdateTeachingActivitySchema = z.object({
  ...
  content: TeachingActivityContentSchema.optional(),
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!resource || resource.resource_type !== 'TEACHING_ACTIVITY') {
      return NextResponse.json({ error: 'Teaching activity not found' }, { status: 404 });
    }
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!existing || existing.resource_type !== 'TEACHING_ACTIVITY') {
      return NextResponse.json({ error: 'Teaching activity not found' }, { status: 404 });
    }
    ...
    const parsed = UpdateTeachingActivitySchema.safeParse(rawBody);
